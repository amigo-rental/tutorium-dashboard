      ...
      <CardBody className="p-5">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            ...
            {subtitle && (
              <p className="text-default-500 font-medium text-xs mt-1">{subtitle}</p>
            )}
          </div>
          {icon && (
...
export function CourseCard({
  ...
  const handleCourseAction = (action: string) => {
    switch (action) {
      case 'details':
        ...
        window.open(`/courses/${title.toLowerCase().replace(/\s+/g, '-')}`, '_blank');
        break;
      case 'materials':
      ...
      case 'teacher':
        // Contact teacher
        window.open(`mailto:${teacher.toLowerCase().replace(/\s+/g, '.')}@tutorium.io`, '_blank');
        break;
      case 'schedule':
  ...
  return (
    <div className={`relative bg-gradient-to-br ${getBgByProgress(progressPercent)} border border-slate-200/50 rounded-3xl p-6 group hover:shadow-2xl hover:shadow-slate-300/25 transition-all duration-500 overflow-hidden`}>
      {/* Background decoration */}
      <div className="absolute top-0 right-0 w-32 h-32 opacity-5">
        <div className={`w-full h-full bg-gradient-to-br ${getGradientByProgress(progressPercent)} rounded-full translate-x-8 -translate-y-8`}></div>
      </div>
      ...
      <div className="relative z-10">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              {coverImage ? (
                ...
                <div className={`w-12 h-12 bg-gradient-to-br ${getGradientByProgress(progressPercent)} rounded-2xl flex items-center justify-center text-white font-semibold text-lg`}>
                  {title.charAt(0)}
                </div>
              )}
              <div>
        ...
        <div className="mb-6">
          <div className="flex justify-between items-center mb-3">
            <span className="text-slate-800 font-bold text-sm">Course Progress</span>
                            <span className="font-semibold text-lg text-slate-900">{progressPercent}%</span>
          </div>
        ...
        {nextLesson && (
          <div className="bg-white/60 backdrop-blur-sm rounded-2xl p-4 mb-6 border border-white/50">
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-slate-800 font-bold text-sm">Следующий урок</span>
            </div>
          ...
          <Button 
            ...
            style={{
              backgroundColor: progressPercent >= 80 ? '#10b981' : 
                              progressPercent >= 60 ? '#3b82f6' : 
                              progressPercent >= 40 ? '#8b5cf6' : '#f59e0b'
            }}
          >
            Перейти в курс
          </Button>
              ...
              <DropdownItem key="details" className="py-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#007EFB]/10 rounded-lg flex items-center justify-center">
                    <svg className="w-4 h-4 text-[#007EFB]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                  </div>
                  <div>
                    <p className="font-bold text-black text-sm">Детали курса</p>
                    <p className="text-black/60 font-medium text-xs">Просмотр полной информации</p>
                  </div>
                </div>
              </DropdownItem>
              ...
              <DropdownItem key="materials" className="py-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#EE7A3F]/10 rounded-lg flex items-center justify-center">
                    <svg className="w-4 h-4 text-[#EE7A3F]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                  </div>
                  <div>
                    <p className="font-bold text-black text-sm">Материалы курса</p>
                    <p className="text-black/60 font-medium text-xs">Скачать учебные материалы</p>
                  </div>
                </div>
              </DropdownItem>
              ...
              <DropdownItem key="teacher" className="py-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#FDD130]/10 rounded-lg flex items-center justify-center">
                    <svg className="w-4 h-4 text-[#FDD130]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z" />
                    </svg>
                  </div>
                  <div>
                    <p className="font-bold text-black text-sm">Связаться с учителем</p>
                    <p className="text-black/60 font-medium text-xs">Написать {teacher}</p>
                  </div>
                </div>
              </DropdownItem>
              ...
              <DropdownItem key="schedule" className="py-3">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#00B67A]/10 rounded-lg flex items-center justify-center">
                    <svg className="w-4 h-4 text-[#00B67A]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                    </svg>
                  </div>
                  <div>
                    <p className="font-bold text-black text-sm">Расписание занятий</p>
                    <p className="text-black/60 font-medium text-xs">Просмотр календаря курса</p>
                  </div>
                </div>
              </DropdownItem>
                ...
                <DropdownItem key="certificate" className="py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-black/60 font-medium text-xs">Скачать сертификат об окончании</p>
                    </div>
                  </div>
                </DropdownItem>
              ) : null}
        ...
        {lesson ? (
          <div className="space-y-4">
            <div>
              ...
              <p className="text-default-600 font-medium text-sm">with {lesson.teacher}</p>
          ...
          <div className="text-center py-8">
            <div className="w-16 h-16 bg-default-100 rounded-lg flex items-center justify-center mx-auto mb-4">
              <svg className="w-8 h-8 text-default-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </div>
            <p className="text-default-600 font-medium text-sm">No upcoming lessons</p>
            <Button
              size="sm"
              variant="light"
    ...
    <Card>
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between w-full">
          ...
          <Link href="#" color="primary" size="sm" className="font-medium">
            View all
          </Link>
        </div>
      </CardHeader>
          ...
          {activities.map((activity) => (
            <div key={activity.id} className="flex gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                {activity.teacher && (
                  <p className="text-default-500 font-medium text-xs">by {activity.teacher}</p>
                )}
                <p className="text-default-500 font-medium text-xs mt-1">{activity.timestamp}</p>
              </div>
            </div>
          ))}
        </div>
